...
class WebAPI():
    def __init__(self,host:str='127.0.0.1',port:int=8000):
        self.host = host
        self.port = port
    async def async_get_request(self,endpoint='',**kwargs):
        try:
...
def extract_html_main_content(html):
    doc = Document(html)
...
def prepare_clickable(text):
    regex = r'\[\d+?\]'
    numbers = re.findall(regex,text)
    items = re.split(regex,text)
    return '\n'.join([f'{number}{item.replace('\n',' ').strip()}' for number,item in zip(numbers,items[-len(numbers):])])
