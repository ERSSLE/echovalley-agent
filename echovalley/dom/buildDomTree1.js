(
    ...
    /**
     * Enhanced function to check if an element is interactive, with special handling for search results
     */
    function isInteractiveElement(element) {
        ...
        // Enhanced search result detection - common classes and patterns for search engines
        const searchResultPatterns = [
            // Baidu search result patterns
            (el) => el.classList.contains("result") || el.classList.contains("c-container"),
            (el) => el.classList.contains("t") && el.tagName.toLowerCase() === "h3",
            (el) => el.classList.contains("c-showurl") || el.classList.contains("c-cache"),

            // Bing search result patterns
            (el) => el.classList.contains("b_algo") || el.classList.contains("b_result"),
            (el) => el.classList.contains("b_title") || el.classList.contains("b_caption"),

            // Google search result patterns
            (el) => el.classList.contains("g") && (el.classList.contains("rc") || el.classList.contains("r")),
            (el) => el.classList.contains("r") && el.querySelector("a"),

            // General search result patterns
            (el) => el.hasAttribute("data-rank") || el.hasAttribute("data-pos"),
            (el) => el.id && (el.id.includes("result") || el.id.includes("item")),

            // News/article cards
            (el) => el.classList.contains("news-card") || el.classList.contains("article-item"),

            // E-commerce product cards
            (el) => el.classList.contains("product") || el.classList.contains("item-card"),

            // Suggestion items
            (el) => el.classList.contains("suggestion") || el.classList.contains("autocomplete-item"),

            // Navigation items
            (el) => el.classList.contains("nav-item") || el.classList.contains("menu-item"),
        ];

        // Check if element matches any search result pattern
        const isSearchResultItem = searchResultPatterns.some(pattern => pattern(element));
        ...
        const hasInteractiveRole =
            ...
            element.hasAttribute("data-url") || // Common in search results
            element.hasAttribute("data-href") || // Common in search results
            element.hasAttribute("data-link") || // Common in search results
            isSearchResultItem; // Include search result items
        ...
        const hasClickHandler =
            ...
            element.hasAttribute("jsaction"); // Google uses jsaction
        ...
        const hasAriaProps =
            ...
            element.hasAttribute("aria-label") || // More inclusive ARIA check
        ...
        // Enhanced checks for search result pages
        const isSearchRelated =
            // Check for common search result container patterns
            (element.parentElement && (
                element.parentElement.classList.contains("result") ||
                element.parentElement.classList.contains("search-result") ||
                element.parentElement.classList.contains("b_algo") ||
                element.parentElement.classList.contains("g")
            )) ||
            // Check for links with search-related text
            (tagName === "a" && (
                element.textContent.toLowerCase().includes("搜索") ||
                element.textContent.toLowerCase().includes("search") ||
                element.textContent.toLowerCase().includes("结果") ||
                element.textContent.toLowerCase().includes("result")
            )) ||
            // Check for elements with search-related attributes
            element.hasAttribute("data-sigil") || // Facebook mobile
            element.hasAttribute("data-ptn") || // Some search engines
            element.classList.toString().includes("search") ||
            element.classList.toString().includes("result");
